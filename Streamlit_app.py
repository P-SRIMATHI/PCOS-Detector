...
def preprocess_data(df):
    ...
    selected_columns = [col for col in ["AMH", "betaHCG", "FSH_mIUmL"] if col in available_columns]
    if len(selected_columns) < 3:
        st.warning(f"Using available features: {selected_columns}")
    
    if not selected_columns:
        st.error("No valid predictor columns found in dataset! Please check the data.")
        return None, None, None, None
    
    X = df[selected_columns]
    y_column = "PCOS_YN" if "PCOS_YN" in available_columns else "PCOS_(Y/N)" if "PCOS_(Y/N)" in available_columns else None
    
    if not y_column:
        st.error("No PCOS target column found in dataset!")
        return None, None, None, None
