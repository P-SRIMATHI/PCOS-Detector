...
with st.expander("2. Data Visualization 📊"):
    ...
    st.pyplot(fig)  # Ensure to render the plot
    ...
    ax.set_xticklabels(selected_features, rotation=45)  # Rotate labels for readability
    st.pyplot(fig)  # Ensure to render the plot
...
with st.expander("3. Health Gamification 🏅"):
...
# 4. Trivia Quiz
with st.expander("4. Trivia Quiz 🧠"):
    st.header("Test Your Knowledge About PCOS!")
    ...
    questions = {
        ...
        "What is the primary treatment for PCOS?": ["Birth control pills", "Antibiotics", "Painkillers"]
    }
...
# 5. Community Support
with st.expander("5. Community Support 💬"):
    st.header("Share Your Experience or Ask a Question")
...
# 6. Chatbot
with st.expander("6. Chatbot 🤖"):
    st.header("Ask Anything About PCOS!")
